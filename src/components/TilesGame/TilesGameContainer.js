...
class TilesGameContainer extends React.Component {
  ...
  prepareCards = () => {
    let cardsContent = getRandomElements(
      (this.props.fieldHeight * this.props.fieldWidth) / 2,
      this.props.cardsImages
  ...
  handleRound = () => {
    //Logic, that is provided when second picture was picked
    const { cards, roundFinished, setCardsClosed, setCardsSolved } = this.props;
    const { firstCard, secondCard } = this.state;
    if (cards[firstCard].content === cards[secondCard].content) {
      setCardsSolved(firstCard, secondCard);
      this.setState({ firstCard: null, secondCard: null });
      roundFinished(true);
    } else {
      roundFinished(false);
      setCardsClosed(firstCard, secondCard);
      this.setState({ firstCard: null, secondCard: null });
    }
  };
  ...
  componentDidUpdate(prevProps) {
    prevProps.cardsSolved !== this.props.cardsSolved &&
      this.props.cardsSolved === this.props.cards.length &&
